...
```bash
# Настройка алиасов (ОБНОВЛЕНО для безопасности)
mc alias set local https://s3.svetu.rs production_backend xK9mNjR3tP5wQ2aLbV7cH8dS
mc alias set contabo https://eu2.contabostorage.com 39e2e4987c6c4c9926c9b24bca119cd0 a479932e4af2c29b16049223b3e54d42

# ⚠️ ВАЖНО: Прямой доступ к портам больше НЕ РАБОТАЕТ из соображений безопасности
# НЕ используйте: mc alias set local http://194.163.132.116:9000
# Используйте только: https://s3.svetu.rs
...
1. **Сетевая безопасность:**
   - ✅ MinIO порты (9000/9001) заблокированы для внешнего доступа
   - ✅ Доступ только через nginx reverse proxy (HTTPS)
   - ✅ UFW firewall с правилами DENY для прямого доступа

2. **Защита данных:**
   - ✅ Все production bucket'ы установлены в режим PRIVATE
   - ✅ Публичные bucket'ы больше не доступны анонимно  
   - ✅ TLS для всех соединений с HSTS заголовками

3. **Заголовки безопасности:**
   - ✅ Strict-Transport-Security (HSTS)
   - ✅ X-Content-Type-Options: nosniff
   - ✅ X-Frame-Options: DENY (консоль)
   - ✅ Referrer-Policy: strict-origin-when-cross-origin

4. **Разделение доступов:**
   - Production приложение → только через HTTPS API
   - Backup scripts → доступ к Contabo через безопасные ключи
   - Dev окружение → изолированные бакеты
   - Admin доступ → только через защищенную консоль

5. **Ротация ключей:**
   - Каждые 90 дней для Contabo
   - Каждые 30 дней для приложений
   - Защищенное хранение через Docker secrets
