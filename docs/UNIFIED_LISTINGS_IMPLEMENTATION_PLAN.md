...
### Проблема: Unified OpenSearch индекс не синхронизирован

**Обнаружено:** 2025-10-11

**Текущее состояние:**
- OpenSearch `c2c_listings` индекс: 9 документов ✅
- OpenSearch `b2c_products` индекс: 5 документов ✅
- OpenSearch `unified_listings` индекс: **9 документов** ⚠️ (ожидалось 14)

**Возможные причины:**
1. Индекс `unified_listings` НЕ автоматически синхронизируется с отдельными индексами
2. При создании C2C/B2C элементов индексация идёт только в свой индекс, но не в unified
3. Возможно, unified search вообще НЕ использует индекс `unified_listings`

**Действия для проверки:**

#### 1️⃣ Проверить unified search handler
```bash
# Посмотреть какой индекс используется в unified search
grep -r "unified_listings" backend/internal/proj/global/
grep -r "SearchUnified" backend/internal/proj/global/
```

**Варианты реализации unified search:**
- **Вариант А:** Используется отдельный индекс `unified_listings` (нужна синхронизация)
- **Вариант Б:** Запросы идут напрямую к `c2c_listings` + `b2c_products` (unified_listings - рудимент)

#### 2️⃣ Если используется unified_listings индекс

**Решение:** Настроить автоматическую переиндексацию при создании элементов.

**Изменения в коде:**

```go
// backend/internal/proj/c2c/service/marketplace.go
// После строки 239 (IndexListing в c2c_listings)

// Также индексируем в unified индекс
if unifiedSearchRepo != nil {
    if err := unifiedSearchRepo.IndexListing(ctx, fullListing); err != nil {
        log.Printf("ERROR: Ошибка индексации в unified_listings: %v", err)
    }
}
```

```go
// backend/internal/proj/b2c/service/product_service.go
// После строки 367 (indexProductWithVariants)

// Также индексируем в unified индекс
if unifiedSearchRepo != nil {
    go unifiedSearchRepo.IndexProduct(ctx, product)
}
```

**Создать скрипт переиндексации:**
```bash
#!/bin/bash
# Переиндексация unified_listings из обоих индексов
python3 backend/reindex_unified.py
```

#### 3️⃣ Если НЕ используется unified_listings индекс

**Решение:** Удалить рудимент.

```bash
# Удалить индекс unified_listings из OpenSearch
curl -X DELETE "http://localhost:9200/unified_listings"

# Удалить упоминания из кода (если есть)
grep -r "unified_listings" backend/ | grep -v ".md"
```

**Обновить документацию:**
- Указать, что unified search работает через прямые запросы к c2c + b2c индексам
- Убрать упоминания unified_listings индекса из архитектуры

#### 4️⃣ Проверить текущую реализацию unified search

```bash
# Найти handler unified search
cat backend/internal/proj/global/handler/unified_search.go | grep -A 20 "func.*Search"

# Найти repository unified search
find backend/internal/proj/global/storage -name "*.go" -exec grep -l "unified" {} \;
```

**Ожидаемый результат:**
- Если handler делает запросы к двум индексам отдельно → unified_listings не нужен
- Если handler использует unified_listings → нужна синхронизация
...
|--------|-----------|-------|
| Проверить unified search handler | 🔴 Высокий | 30 мин |
| Если используется - настроить синхронизацию | 🟡 Средний | 2 часа |
| Если не используется - удалить рудимент | 🟢 Низкий | 30 мин |
| Переиндексировать unified_listings (если нужно) | 🟡 Средний | 15 мин |
| Обновить документацию | 🟢 Низкий | 30 мин |
