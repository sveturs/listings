...
|---------|---------|---------|--------|
| Правильное использование middleware | 75% | 95%+ | 🟡 |
| Использование helper функций | 65% | 90%+ | 🟠 |
| Модулей без критических проблем | 15/18 | 18/18 | 🔴 |
| Критических уязвимостей | 3 | 0 | 🔴 |
...
**Описание:**
4 модуля используют неправильный ключ `"userID"` вместо `"user_id"`, что приводит к тому, что userID всегда nil.

**Затронутые файлы:**
- [ ] `backend/internal/proj/recommendations/handler.go`
- [ ] `backend/internal/proj/global/handler/unified_search.go`
- [ ] `backend/internal/proj/marketplace/handler/category_detector_handler.go`
- [ ] `backend/internal/proj/marketplace/handler/admin_translations.go`
...
**Описание:**
Модуль subscriptions использует старый middleware вместо библиотеки auth.

**Затронутые файлы:**
- [ ] `backend/internal/proj/subscriptions/handler/routes.go` (строки 16, 26)
...
**Описание:**
Admin routes используют AdminRequired без RequireAuth, что создаёт потенциальную уязвимость.

**Затронутые файлы:**
- [ ] `backend/internal/proj/marketplace/handler/handler.go:422`
