...
use_ssl: true
# ⚠️  ВАЖНО: Используйте переменные окружения для хранения ключей!
```
...
### 🚨 ВАЖНОЕ ИЗМЕНЕНИЕ БЕЗОПАСНОСТИ:
```yaml
# ❌ ПРЯМОЙ ДОСТУП БОЛЬШЕ НЕ РАБОТАЕТ (заблокирован из соображений безопасности)
# endpoint: http://194.163.132.116:9000  

# ✅ ИСПОЛЬЗУЙТЕ ТОЛЬКО ЗАЩИЩЕННЫЕ ENDPOINTS:
endpoint: https://s3.svetu.rs
# Все подключения теперь только через HTTPS!
```

### ⚠️ ИЗМЕНЕНИЕ: Публичные bucket'ы больше НЕ ПУБЛИЧНЫЕ
```
# ❌ БОЛЬШЕ НЕ РАБОТАЕТ (приватные из соображений безопасности):
# https://s3.svetu.rs/production-listings/image.jpg → 403 Forbidden
# https://s3.svetu.rs/development-listings/image.jpg → 403 Forbidden

# ✅ ИСПОЛЬЗУЙТЕ PRESIGNED URL ДЛЯ ДОСТУПА К ФАЙЛАМ:
# Генерируйте presigned URLs через API для безопасного доступа
# См. примеры кода ниже для создания presigned URLs
```
...
### Production бакеты:
| Бакет | Назначение | Доступ | Lifecycle | Версионирование |
|-------|------------|--------|-----------|----------------|
| `production-listings` | Фото объявлений | ✅ **ПРИВАТНЫЙ** | 30 дней → архив | ✅ Включено |
| `production-storefronts` | Витрины магазинов | ✅ **ПРИВАТНЫЙ** | Постоянно | ✅ Включено |
| `production-chat-files` | Файлы из чатов | ✅ **ПРИВАТНЫЙ** | 90 дней → удаление | ✅ Включено |
| `production-user-documents` | Документы | ✅ **ПРИВАТНЫЙ** | Постоянно | ✅ Включено |

### Development бакеты:
| Бакет | Назначение | Доступ | Lifecycle |
|-------|------------|--------|-----------|
| `development-listings` | Тестовые фото | ✅ **ПРИВАТНЫЙ** | 30 дней → удаление |
| `development-storefronts` | Тест витрины | ✅ **ПРИВАТНЫЙ** | 7 дней → удаление |
...
// Конфигурация клиента (ОБНОВЛЕНО для безопасности)
...
async function uploadFile(file, listingId) {
  ...
  // ⚠️ ИЗМЕНЕНИЕ: Файлы теперь приватные, используйте presigned URL
  return await createPresignedUrl('production-listings', key);
}
...
# Конфигурация (ОБНОВЛЕНО для безопасности)
s3 = boto3.client(
    's3',
    endpoint_url='https://s3.svetu.rs',
...
def upload_listing_image(file_path, listing_id):
    ...
    # ⚠️ ИЗМЕНЕНИЕ: Файлы приватные, возвращаем presigned URL
    return create_presigned_url('production-listings', key)
...
func main() {
    // Инициализация (ОБНОВЛЕНО для безопасности)
    minioClient, err := minio.New("s3.svetu.rs:443", &minio.Options{
...
// Создание клиента (ОБНОВЛЕНО для безопасности)
$s3 = new S3Client([
    'version' => 'latest',
    'region'  => 'us-east-1',
    'endpoint' => 'https://s3.svetu.rs',
...
function uploadListingImage($filePath, $listingId) {
    ...
    try {
        ...
        // ⚠️ ИЗМЕНЕНИЕ: Используйте presigned URL для доступа к приватным файлам
        return getPresignedUrl('production-listings', $key);
...
# Настройка (ОБНОВЛЕНО для безопасности)
./mc alias set svetu https://s3.svetu.rs \
...
### cURL для проверки:
```bash
# Проверка доступности (ОБНОВЛЕНО)
# ❌ ПРЯМОЙ ДОСТУП БОЛЬШЕ НЕ РАБОТАЕТ:
# curl -I http://194.163.132.116:9000/minio/health/live

# ✅ ИСПОЛЬЗУЙТЕ HTTPS ENDPOINT:
...
### 🔒 Безопасность (ОБНОВЛЕНО):
1. **НИКОГДА** не коммитьте credentials в репозиторий
2. Используйте переменные окружения для хранения ключей
3. **ОБЯЗАТЕЛЬНО** используйте HTTPS endpoints (не HTTP!)
4. **ВСЕ файлы теперь приватные** - используйте presigned URLs
5. Ротируйте ключи каждые 90 дней
6. **НЕ используйте** прямое подключение к портам 9000/9001
7. Все запросы только через nginx reverse proxy
...
1. Доступность сервиса: `curl https://s3.svetu.rs/`
...
### Недавно реализовано:
- ✅ Docker Secrets для безопасного хранения паролей
- ✅ Fail2ban для защиты от брутфорса
- ✅ UFW Firewall с правильными правилами
- ✅ Версионирование объектов для production
- ✅ Lifecycle политики для автоматической очистки
- ✅ Автоматические ежедневные бэкапы
- ✅ Ротация логов

### Ближайшие улучшения:
- ⏳ SSL сертификат для консоли (нужна DNS запись)
