// frontend/hostel-frontend/src/pages/marketplace/ListingDetailsPage.tsx
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { useTranslation } from 'react-i18next';
import { useParams, useNavigate, Link } from 'react-router-dom';
import {
    MapPin, Calendar, Heart, ChevronLeft, ChevronRight,
    Info, Tag, Settings, BarChart, Check, DollarSign,
    Filter, Zap, Percent, PencilLine, Trash2, Store, X
} from 'lucide-react';
import {
...
interface ListingAttribute {
    ...
    display_name: string;
    display_value: string; // Приводим к string для совместимости
...
// Импорты удалены, так как перенесены в начало файла

// Дополнительные интерфейсы
// Используем Record<string, string | undefined> для useParams
...
// Явно не переопределяем Window.ENV, используем имеющийся интерфейс
// Если потребуется добавить новые поля, сделаем это в глобальном types/global.d.ts
...
const ListingDetailsPage: React.FC = () => {
    ...
    const { id } = useParams();
    if (!id) throw new Error('Listing ID not found');
    ...
    const getAttributeIcon = (attributeName: string): JSX.Element => {
    ...
    const renderDiscountInfo = (): JSX.Element | null => {
    ...
    const formatAttributeValue = (attr: ListingAttribute): string => {
        ...
        if (attr.attribute_type === 'boolean') {
            return attr.display_value === 'true' || attr.boolean_value === true ?
        ...
        if (currentLang === 'ru') {
            return String(attr.display_value);
        }
        ...
        if ((attr.attribute_type === 'number' || attr.numeric_value !== undefined) && attr.unit) {
            ...
            const unitTranslations: {[unit: string]: {[lang: string]: string}} = {
    ...
    const debugAttribute = (attr: ListingAttribute): void => {
    ...
    const translateKnownValue = (value: string, translations: {[key: string]: {[lang: string]: string}}): string => {
    ...
    const findCategoryPath = useCallback((categoryId: string | number, categoriesTree: any[]): CategoryPath[] => {
        const path: CategoryPath[] = [];

        const findPath = (id: string | number, categories: any[]): boolean => {
    ...
    const fetchListing = useCallback(async (): Promise<void> => {
        try {
            ...
            // Отладка изображений
            console.group('Данные изображений:');
            console.log('Количество изображений:', listingData.images?.length);
            console.log('Полные данные изображений:', JSON.stringify(listingData.images));
            if (listingData.images && listingData.images.length > 0) {
                const firstImage = listingData.images[0];
                console.log('Тип первого изображения:', typeof firstImage);
                if (typeof firstImage === 'object') {
                    console.log('Ключи первого изображения:', Object.keys(firstImage));
                    console.log('public_url:', firstImage.public_url);
                    console.log('Тип public_url:', typeof firstImage.public_url);
                }
            }
            console.groupEnd();
            ...
            if (listingData.attributes && listingData.attributes.length > 0) {
                ...
                listingData.attributes.forEach((attr: ListingAttribute) => {
                    debugAttribute(attr);
                });
                console.groupEnd();
            } else {
    ...
    // Функция получения URL изображения - оптимизированная версия
    const getImageUrl = (image: string | ListingImageObject | undefined): string => {
        if (!image) {
        ...
        if (typeof image === 'string') {
            console.log('Processing string image path:', image);
        ...
        if (typeof image === 'object' && image.file_path) {
            console.log('Processing image object:', image.file_path);
            ...
            // Приоритет 2: Формируем URL на основе типа хранилища и пути
            if (image.storage_type === 'minio' || (image.file_path && image.file_path.includes('listings/'))) {
                // Путь для MinIO может содержать префикс listings/ или не содержать
                const path = image.file_path.includes('listings/') 
                    ? image.file_path 
                    : `listings/${image.file_path}`;
            ...
            // Приоритет 3: Локальное хранилище
    ...
    const getImagePaths = (): string[] => {
        ...
        console.log('Getting image paths for gallery. Images:', listing.images);
        // Используем нашу оптимизированную функцию getImageUrl для каждого изображения
        
        // Результирующий массив URL-ов изображений
        const imagePaths: string[] = [];

        for (const img of listing.images) {
            console.log('Processing image', img);
            
            // Получаем URL с использованием универсальной функции
