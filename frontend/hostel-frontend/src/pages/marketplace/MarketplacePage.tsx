// frontend/hostel-frontend/src/pages/marketplace/MarketplacePage.tsx
...
const MobileListingGrid: React.FC<MobileListingGridProps> = ({ listings, viewMode = 'grid' }) => {
...
const MarketplacePage: React.FC = () => {
    ...
    const [mapViewActive, setMapViewActive] = useState<boolean>(false);
    const [userLocationState, setUserLocationState] = useState<LocationState | null>(null);
    ...
    const fetchListings = useCallback(async (currentFilters: FilterState = filters, isLoadMore: boolean = false) => {
        try {
...
// Обновляем информацию о пагинации
        ...
        setFilters(prev => {
            ...
            if (updated.attributeFilters) {
                Object.entries(updated.attributeFilters).forEach(([attrKey, attrValue]) => {
                    if (attrValue) {
                        nextParams.set(`attr_${attrKey}`, String(attrValue));
                    }
                });
            }
    ...
    const handleToggleMapView = useCallback(() => {
        ...
        if (mapViewActive) {
            ...
            if (userLocation) {
                nextParams.set('latitude', String(userLocation.lat));
                nextParams.set('longitude', String(userLocation.lon));
            ...
            // Если нет данных в userLocation, но есть в фильтрах
                ...
                nextParams.set('latitude', String(filters.latitude));
                nextParams.set('longitude', String(filters.longitude));
                nextParams.set('distance', filters.distance || '5km');
            }
    ...
    useEffect(() => {
        const handleCityChange = (event: Event) => {
            // Приведение типа для custom event
            ...
            const { lat, lon, city, country } = customEvent.detail;
            ...
            setSearchParams(prev => {
                ...
                nextParams.set('latitude', String(lat));
                nextParams.set('longitude', String(lon));
    ...
    const findCategoryPath = (categoryId: string | number, categoriesTree: Category[]): Category[] => {
        if (!categoryId || !categoriesTree || categoriesTree.length === 0) {
            return [];
        }

        // Создаем плоскую карту всех категорий для быстрого поиска
        const categoryMap = new Map<string, Category>(); // Используем нативный Map
        ...
        while (currentId) {
            ...
            currentId = category.parent_id ? String(category.parent_id) : '';
        }
