// frontend/hostel-frontend/src/components/marketplace/AttributeFields.tsx
...
// Тип атрибута, который может быть получен с сервера
interface AttributeDefinition {
    id: number | string;
    name: string;
    display_name: string;
    attribute_type: 'text' | 'number' | 'select' | 'boolean' | 'multiselect';
    options?: string | {
...
const AttributeFields: React.FC<AttributeFieldsProps> = ({ categoryId, value = [], onChange, error }) => {
    const { t, i18n } = useTranslation('marketplace');
    const [attributes, setAttributes] = useState<AttributeDefinition[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [values, setValues] = useState<AttributeValue[]>(value);
    const [requestInProgress, setRequestInProgress] = useState<boolean>(false);

    // Отслеживаем предыдущую категорию для предотвращения повторных запросов
    const prevCategoryIdRef = useRef<number | string | null>(null);
    // Отслеживаем, были ли установлены внешние значения
    const hasSetExternalValues = useRef<boolean>(false);

    // Функция для получения переведенного значения опции атрибута
    const getTranslatedOptionValue = (attribute: AttributeDefinition, value: string): string => {
    ...
    useEffect(() => {
        ...
        const fetchAttributes = async (): Promise<void> => {
            ...
            try {
                ...
                if (response.data?.data) {
                    ...
                    response.data.data.forEach((attr: AttributeDefinition) => {
                    ...
                    const needReset = !values.length ||
                        ...
                        !response.data.data.some((attr: AttributeDefinition) =>
                            values.some(val => val.attribute_id === attr.id));

                    if (needReset) {
                        console.log("Инициализация значений атрибутов по умолчанию");
                        const initialValues: AttributeValue[] = response.data.data.map((attr: AttributeDefinition) => {
                                    ...
                                    if (attr.options) {
                                        try {
                                            ...
                                            if (options.min !== undefined) {
                                                numDefaultValue = parseFloat(options.min.toString());
                                            }
                                        } catch (e) {
        ...
        fetchAttributes();
    }, [categoryId, i18n.language, attributes.length, onChange, values, value.length]);
    ...
    useEffect(() => {
        ...
        if (value && value.length > 0) {
            ...
            const processedValues: AttributeValue[] = [];
            const seen: Record<string | number, boolean> = {}; // Для отслеживания дубликатов
            ...
            value.forEach(attr => {
                ...
                const processed: AttributeValue = { ...attr };
    ...
    const getDefaultValueForType = (type: string): string | number | boolean | string[] => {
        switch (type) {
            case 'text':
            case 'select':
                return '';
    ...
    const handleAttributeChange = useCallback((attributeId: number | string, newValue: any) => {
        ...
        const updatedValues = values.map(attr => {
            if (attr.attribute_id === attributeId) {
                ...
                const updatedAttr: AttributeValue = { ...attr, value: newValue };
                ...
                if (attribute && attribute.attribute_type === 'boolean') {
                    ...
                    if (attribute.name === 'mileage') {
                        ...
                        const numRooms = updatedAttr.numeric_value || 0;
    ...
    const getTranslatedName = (attribute: AttributeDefinition): string => {
        if (!attribute) return '';
    ...
    const renderField = (attribute: AttributeValue): React.ReactNode => {
        ...
        switch (attr.attribute_type) {
            ...
            case 'number': {
                ...
                let options: { min?: number; max?: number; step?: number } = {};
                try {
                    options = attr.options ? 
                        (typeof attr.options === 'string' ? JSON.parse(attr.options) : attr.options) : 
                        {};
                } catch (e) {
                    options = {};
                }
                ...
                let inputAdornment: string | null = null;
                let valueSuffix = '';
                ...
                if (attr.name === 'year') {
                    ...
                    return (
                        <FormControl fullWidth required={isRequired}>
                            ...
                            <Select
                                value={parseInt(attrValue as string) || currentYear}
                                onChange={(e: SelectChangeEvent<number>) => 
                                    handleAttributeChange(attr.id, parseInt(e.target.value as string))
                                }
                                label={displayName}
                            >
                                {years.map((year) => (
                ...
                return (
                    <Box sx={{ width: '100%' }}>
                        ...
                        {useSlider && (
                            <Slider
                                value={parseFloat(attrValue as string) || min}
            ...
            case 'multiselect': {
                ...
                let options: string[] = [];
                try {
                    const parsedOptions = attr.options ? 
                        (typeof attr.options === 'string' ? JSON.parse(attr.options) : attr.options) : 
                        {};
                    options = parsedOptions.values || [];
                } catch (e) {
