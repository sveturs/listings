...
const SimilarListings: React.FC<SimilarListingsProps> = ({ listingId, initialLimit = 8 }) => {
    ...
    const fetchSimilarListings = useCallback(async (isLoadMore = false) => {
        try {
            ...
            if (response.data && response.data.data) {
                const newListings = response.data.data as Listing[];
    ...
    */
    ...
    const getImageUrl = (listing: Listing): string => {
        ...
        const mainImage = listing.images.find(img => (img as ImageObject).is_main) || listing.images[0];
    ...
    const renderDiscountBadge = (listing: Listing): React.ReactNode => {
    ...
    const handleCardClick = (id: number | string): void => {
