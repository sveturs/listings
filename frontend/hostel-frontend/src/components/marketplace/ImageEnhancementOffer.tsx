// frontend/hostel-frontend/src/components/marketplace/ImageEnhancementOffer.tsx
...
interface EnhanceApiResponse {
    data: {
        success: boolean;
        enhanced_images: EnhancedImageResponse[];
    };
}

interface EnhancePreviewResponse {
    data: {
        original: string;
        enhanced: string;
        price: number;
...
interface ImageEnhancementOfferProps {
    images: ImageObject[];
    onEnhanced: (enhancedImages: ImageObject[]) => void;
}

const ImageEnhancementOffer: React.FC<ImageEnhancementOfferProps> = ({ images, onEnhanced }) => {
    ...
    const handleRequestPreview = async (imageIndex: number): Promise<void> => {
        ...
        try {
            ...
            formData.append('image', images[imageIndex].file as File);

            const response = await axios.post<EnhancePreviewResponse>('/api/v1/marketplace/enhance-preview', formData, {
                headers: {
    ...
    const handleEnhanceAll = async (): Promise<void> => {
        setProcessing(true);
        ...
        try {
            ...
            const response = await axios.post<EnhanceApiResponse>('/api/v1/marketplace/enhance-images', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
