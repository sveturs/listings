// frontend/hostel-frontend/src/components/marketplace/AttributeFilters.tsx
...
// Type definitions for attribute structure
...
interface AttributeOptionTranslations {
    [language: string]: {
...
interface Attribute {
    ...
    option_translations?: AttributeOptionTranslations;
}
...
interface AttributeFiltersProps {
    ...
    onAttributesLoaded?: (hasAttributes: boolean) => void;
}
...
const AttributeFilters: React.FC<AttributeFiltersProps> = ({ 
    ...
    const [attributeFilters, setAttributeFilters] = useState<Record<string, any>>({ ...filters });
    ...
    const getTranslatedOptionValue = (attribute: Attribute | undefined, value: string): string => {
        // Для отладки
    ...
    const handleFilterChange = useCallback((attributeName: string, value: any) => {
        ...
        if (['rooms', 'floor', 'total_floors', 'area', 'land_area', 'property_type'].includes(attributeName)) {
            ...
            if (typeof value === 'string' && value.includes(',')) {
                ...
                if (!isNaN(numericValue)) {
                    ...
                    const cleanValue = (value as string).replace(/[^\d.,\-]/g, '').replace(',', '.');
    ...
    const handleRangeFilter = useCallback((attributeName: string, minValue: number, maxValue: number) => {
    ...
    useEffect(() => {
        ...
        const fetchAttributeRanges = async (): Promise<void> => {
            try {
        ...
        const fetchAttributes = async (): Promise<void> => {
            try {
                ...
                if (response.status === 200 && response.data?.data) {
                    ...
                    const filterableAttrs = response.data.data.filter((attr: Attribute) => attr.is_filterable);
                    ...
                    filterableAttrs.forEach((attr: Attribute) => {
            ...
            if (['rooms', 'floor', 'total_floors', 'area', 'land_area', 'property_type'].includes(attr.name)) {
                ...
                if (['rooms', 'floor', 'total_floors', 'area', 'land_area'].includes(attr.name)) {
                    ...
                    if (attributeRanges[attr.name]) {
                        ...
                        let defaultOptions: AttributeOption = {};
                ...
                // Специальная обработка для property_type
                    ...
                    const propertyTypes: AttributeOption = {
                        values: [
                            "квартира", 
                            "дом", 
                            "комната",
                ...
                if (attr.name === "rooms" && attr.attribute_type === "select") {
                    const roomOptions: AttributeOption = {
                        values: ["1", "2", "3", "4", "5+"]
                    };
    ...
    const getTranslatedName = (attribute: Attribute): string => {
        if (!attribute) return '';
    ...
    const renderFilter = (attribute: Attribute): React.ReactNode => {
        ...
        switch (attribute.attribute_type) {
            case 'text':
                return (
                    <TextField
                        ...
                        size="small"
                        value={currentValue}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleFilterChange(attributeName, e.target.value)}
                    />
                );
            ...
            case 'number': {
                ...
                let min: number, max: number, step = options.step || 1;
                let valueSuffix = '';
                ...
                return (
                    <Box sx={{ width: '100%', px: 1 }}>
                        ...
                        <Slider
                            value={[parseFloat(String(value[0])) || min, parseFloat(String(value[1])) || max]}
                            onChange={(e, newValue) => {
                                // Только локальное обновление UI
                                setAttributeFilters(prev => ({
                                    ...prev,
                                    [attributeName]: `${(newValue as number[])[0]},${(newValue as number[])[1]}`
                                }));
                            }}
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                            <TextField
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) => {
                                    const newValue: [number, number] = [parseFloat(e.target.value) || min, value[1]];
                            ...
                            <TextField
                                ...
                                value={value[1]}
                                onChange={(e: ChangeEvent<HTMLInputElement>) => {
                                    const newValue: [number, number] = [value[0], parseFloat(e.target.value) || max];
                                    handleRangeFilter(attributeName, newValue[0], newValue[1]);
                                }}
                                InputProps={{
                                    endAdornment: inputAdornment ? (
            ...
            case 'select': {
                ...
                try {
                    ...
                    if (typeof attribute.options === 'string') {
                        ...
                        if ((attribute.options as AttributeOption).values) {
                            if (Array.isArray((attribute.options as AttributeOption).values)) {
                                options = (attribute.options as AttributeOption).values as string[];
                            } else {
                                // Если values существует, но не массив
                                options = [String((attribute.options as AttributeOption).values)];
                            }
                        }
                    }
