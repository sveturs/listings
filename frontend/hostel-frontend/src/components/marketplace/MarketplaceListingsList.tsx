// frontend/hostel-frontend/src/components/marketplace/MarketplaceListingsList.tsx
...
type ColumnType = 'image' | 'title' | 'price' | 'reviews' | 'date' | 'status' | 'promotions';
...
const MarketplaceListingsList: React.FC<MarketplaceListingsListProps> = ({
    listings,
    selectedItems = [],
    onSelectItem = null,
    onSelectAll = null,
    ...
    const [order, setOrder] = useState<'asc' | 'desc'>(initialSortOrder);
    const [orderBy, setOrderBy] = useState<string>(initialSortField);

    // Состояние для хранения данных о рейтингах
    const [listingsWithRatings, setListingsWithRatings] = useState<Listing[]>([]);
    const [loadingRatings, setLoadingRatings] = useState<boolean>(false);

    // Определяем колонки таблицы
    const [columns, setColumns] = useState<ColumnType[]>(['image', 'title', 'price', 'reviews', 'date', 'status', 'promotions']);

    // Функция для получения локализованного текста
    const getLocalizedText = (listing: Listing | undefined, field: string): string => {
        if (!listing) return '';
        ...
        if (currentLanguage === listing.original_language) {
            return listing[field as keyof Listing] as string;
        }
            ...
            try {
                ...
                const ratingPromises = listings.map(listing =>
                    axios.get(`/api/v1/entity/listing/${listing.id}/stats`)
                        .then(response => {
                            const stats = response.data?.data as Stats | undefined;
                        ...
                        .catch(error => {
                            ...
                            return { id: listing.id, review_count: 0, average_rating: 0 } as RatingData;
                        })
                );
    ...
    useEffect(() => {
        ...
        const sortByValue = filters?.sort_by;
        if (sortByValue) {
            const sortParts = sortByValue.split('_');
