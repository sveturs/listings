// frontend/hostel-frontend/src/components/store/CategoryMappingEditor.tsx
...
interface DraggableSystemCategoryProps {
  category: Category;
  onDragStart?: (category: Category) => void;
  language: string;
  isMobileDevice: boolean;
  onMobileSelect?: (category: Category) => void;
}

interface DroppableImportCategoryProps {
  sourceCategory: string;
  sourcePath: string;
  mappedTo?: number;
  formatCategoryName: (categoryId: number) => string;
  onDrop: (sourcePath: string, categoryId: number | null) => void;
  isMobileDevice: boolean;
  onMobileSelect?: (sourcePath: string, e: React.MouseEvent) => void;
}

interface SystemCategoryTreeProps {
  categories: Category[];
...
interface ImportedCategoryTreeProps {
...
interface MobileCategorySelectionModalProps {
  open: boolean;
  onClose: () => void;
...
const getLocalizedCategoryName = (category: Category, language: string): string => {
...
const DraggableSystemCategory: React.FC<DraggableSystemCategoryProps> = ({ 
  category, 
  onDragStart, 
  language, 
  isMobileDevice, 
...
const DroppableImportCategory: React.FC<DroppableImportCategoryProps> = ({ 
  sourceCategory, 
  sourcePath, 
  mappedTo, 
  formatCategoryName, 
...
const SystemCategoryTree: React.FC<SystemCategoryTreeProps> = ({ 
  categories, 
  onCategoryDragStart, 
  language, 
  isMobileDevice, 
  ...
  const handleToggle = (categoryId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    setExpanded(prev => ({
      ...prev,
...
const ImportedCategoryTree: React.FC<ImportedCategoryTreeProps> = ({ 
  categories, 
  mappings, 
  formatCategoryName, 
  onDrop, 
  ...
  // Функция для обработки клика по категории
  const handleToggle = (categoryKey: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setExpanded(prev => ({
      ...prev,
  ...
  return (
    <List disablePadding>
      {Object.entries(categories).map(([parentCategory, parentData]) => {
        const typedParentData = parentData as CategoryNode;
        return (
          <React.Fragment key={parentCategory}>
            ...
            >
              ...
              >
                <ListItemIcon sx={{ minWidth: 36 }}>
                  {Object.keys(typedParentData.children).length > 0 ? (
                ...
                <DroppableImportCategory
                  ...
                  mappedTo={typedParentData.mappedTo}
            ...
            {Object.keys(typedParentData.children).length > 0 && (
              <Collapse in={expanded[parentCategory]} timeout="auto" unmountOnExit>
                <List disablePadding sx={{ pl: 4 }}>
                  {Object.entries(typedParentData.children).map(([childCategory, childData]) => {
                    const typedChildData = childData as CategoryNode;
                    return (
                      <React.Fragment key={`${parentCategory}|${childCategory}`}>
                        ...
                        >
                          ...
                          >
                            <ListItemIcon sx={{ minWidth: 36 }}>
                              {Object.keys(typedChildData.children).length > 0 ? (
                            ...
                            <DroppableImportCategory
                              ...
                              mappedTo={typedChildData.mappedTo}
                              formatCategoryName={formatCategoryName}
                              onDrop={onDrop}
                              isMobileDevice={isMobileDevice}
                              onMobileSelect={onMobileSelect}
                        ...
                        {Object.keys(typedChildData.children).length > 0 && (
                          <Collapse in={expanded[`${parentCategory}|${childCategory}`]} timeout="auto" unmountOnExit>
                            <List disablePadding sx={{ pl: 4 }}>
                              {Object.entries(typedChildData.children).map(([grandchildCategory, grandchildData]) => {
                                const typedGrandchildData = grandchildData as CategoryNode;
                                return (
                                  <ListItem
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <DroppableImportCategory
                                        ...
                                        mappedTo={typedGrandchildData.mappedTo}
                                        formatCategoryName={formatCategoryName}
                                        onDrop={onDrop}
                                        isMobileDevice={isMobileDevice}
                                        onMobileSelect={onMobileSelect}
...
const MobileCategorySelectionModal: React.FC<MobileCategorySelectionModalProps> = ({
  open,
  onClose,
  selectedImportPath,
  categories,
...
const CategoryMappingEditor: React.FC<CategoryMappingEditorProps> = ({ sourceId, onClose, onSave }) => {
  ...
  const [organizedCategories, setOrganizedCategories] = useState<Record<string, CategoryNode>>({});
  const [importedCategoriesLoading, setImportedCategoriesLoading] = useState<boolean>(false);
  const [searchSystemTerm, setSearchSystemTerm] = useState<string>('');
  const [searchImportTerm, setSearchImportTerm] = useState<string>('');

  // Состояние для применения сопоставлений
  const [applyingMappings, setApplyingMappings] = useState<boolean>(false);
  ...
  const [showMobileMapping, setShowMobileMapping] = useState<boolean>(false);
  const [selectedImportPath, setSelectedImportPath] = useState<string | null>(null);
  const [isMobileDevice, setIsMobileDevice] = useState<boolean>(false);
  const [lastMapSuccess, setLastMapSuccess] = useState<boolean>(false);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (categoriesResponse.data.success) {
          ...
          const sortedCategories = [...categoriesResponse.data.data].sort((a: Category, b: Category) =>
  ...
  const filteredCategories = useMemo(() => {
    ...
    const buildCategoryPaths = (categories: Category[]) => {
      const categoryPaths: Record<number, Category[]> = {};
      const categoriesMap: Record<number, Category> = {};
      ...
      const buildPath = (categoryId: number, path: Category[] = []): Category[] => {
  ...
  const filteredImportCategories = useMemo(() => {
    ...
    const filtered: Record<string, CategoryNode> = {};
    ...
    Object.entries(organizedCategories).forEach(([parentKey, parentData]) => {
      ...
      if (parentKey.toLowerCase().includes(searchLower)) {
        filtered[parentKey] = { ...parentData as CategoryNode };
        return;
      }

      // Поиск по дочерним категориям
      const filteredChildren: Record<string, CategoryNode> = {};
      let hasMatchingChildren = false;

      Object.entries((parentData as CategoryNode).children).forEach(([childKey, childData]) => {
        // Если ребенок соответствует поиску
        if (childKey.toLowerCase().includes(searchLower)) {
          filteredChildren[childKey] = { ...childData as CategoryNode };
          hasMatchingChildren = true;
          return;
        }

        // Поиск по внукам
        const filteredGrandchildren: Record<string, CategoryNode> = {};
        let hasMatchingGrandchildren = false;

        Object.entries((childData as CategoryNode).children).forEach(([grandchildKey, grandchildData]) => {
          if (grandchildKey.toLowerCase().includes(searchLower)) {
            filteredGrandchildren[grandchildKey] = { ...grandchildData as CategoryNode };
  ...
  const organizeImportedCategories = (importedCategories: string[]): Record<string, CategoryNode> => {
    ...
    sortedCategories.forEach(category => {
      ...
      if (levels.length === 1) {
        ...
        categoryTree[grandparent].children[parent].children[child] = { children: {}, mapped: false };
      }
    });
  ...
  const markMappedCategories = (categoryTree: Record<string, CategoryNode>, mappings: Record<string, number>): Record<string, CategoryNode> => {
  ...
  const handleCategoryDrop = (sourcePath: string, targetCategoryId: number | null) => {
  ...
  const handleMobileSystemCategorySelect = (category: Category) => {
  ...
  const handleMobileImportCategorySelect = (sourcePath: string, event: React.MouseEvent) => {
  ...
  const formatCategoryName = (categoryId: number): string => {
...
// Экспортируем компонент модального окна
export const CategoryMappingModal: React.FC<CategoryMappingModalProps> = ({ 
  ...
  return (
    // @ts-ignore - Ignoring TypeScript error for Modal component due to MUI API
    <Modal
      open={open}
      onClose={onClose}
