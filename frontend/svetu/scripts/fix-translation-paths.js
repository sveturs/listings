...
const USE_TRANSLATIONS_PATTERN = /const\s+(\w+)\s*=\s*useTranslations\(['"](\w+)\.(\w+)['"]\)/g;
...
function processFile(filePath) {
  ...
  fileContent.replace(USE_TRANSLATIONS_PATTERN, (match, varName, module, submodule) => {
  ...
  replacements.forEach(({ varName, module, submodule }) => {
    console.log(`  Найдено: const ${varName} = useTranslations('${module}.${submodule}')`);
    
    // Заменяем useTranslations
    fileContent = fileContent.replace(
      new RegExp(`const\\s+${varName}\\s*=\\s*useTranslations\\(['"]${module}\\.${submodule}['"]\\)`, 'g'),
    ...
    console.log(`  Заменено на: const ${varName} = useTranslations('${module}')`);
...
function main() {
  ...
  if (processedCount > 0) {
    ...
    console.log('Некоторые сложные случаи могут требовать ручной корректировки.');
  }
}
