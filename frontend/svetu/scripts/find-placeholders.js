...
async function findAllPlaceholders() {
  console.log(
    `${colors.cyan}🔍 Поиск placeholder'ов в переводах...${colors.reset}\n`
  );
  ...
  console.log(
    `${colors.blue}📦 Анализирую ${modules.length} модулей...${colors.reset}\n`
  );
  ...
  for (const module of modules) {
    ...
    for (const lang of LANGUAGES) {
      ...
      try {
        ...
        console.log(
          `${colors.yellow}⚠️  Ошибка чтения: ${filePath}${colors.reset}`
        );
      }
    }
  ...
  if (report.totalPlaceholders === 0) {
    console.log(
      `${colors.green}✨ Placeholder'ы не найдены! Все переводы заполнены.${colors.reset}`
    );
  } else {
    console.log(
      `${colors.yellow}⚠️  Найдено placeholder'ов: ${report.totalPlaceholders}${colors.reset}\n`
    );
    ...
    Object.entries(report.byLanguage)
      ...
      .forEach(([lang, count]) => {
        if (count > 0) {
          ...
          console.log(
            `  ${lang}: ${colors.yellow}${count}${colors.reset} (${percent}%)`
          );
        }
      });
    ...
    examples.forEach(({ module, language, key, value }) => {
      console.log(
        `${colors.yellow}📍 ${module}/${language}/${key}:${colors.reset}`
      );
      console.log(
  ...
  if (report.totalPlaceholders > 0) {
    ...
    const csvContent = report.details
      .map(
        (d) =>
    ...
    console.log(
      `${colors.magenta}📄 CSV отчет сохранен: ${csvPath}${colors.reset}`
    );
  }
  ...
  if (report.totalPlaceholders > 0) {
    ...
    if (
      report.byType['[MISSING_EN]'] ||
      report.byType['[MISSING_RU]'] ||
      report.byType['[MISSING_SR]']
    ) {
      console.log(
