...
async function syncTranslations() {
  console.log(`${colors.cyan}🔄 Начинаю синхронизацию ключей переводов...${colors.reset}\n`);
  ...
  console.log(`${colors.blue}📦 Найдено модулей: ${modules.length}${colors.reset}`);
  console.log(`${colors.blue}🌍 Языки: ${LANGUAGES.join(', ')}${colors.reset}\n`);
  ...
  for (const module of modules) {
    ...
    for (const lang of LANGUAGES) {
      ...
      try {
        ...
        console.log(`${colors.yellow}⚠️  Файл не найден или поврежден: ${filePath}${colors.reset}`);
        translations[lang] = {};
      }
    }
    ...
    for (const lang of LANGUAGES) {
      ...
      if (missingKeys.length > 0) {
        ...
        console.log(`${colors.red}❌ ${module}.json (${lang}): отсутствует ${missingKeys.length} ключей${colors.reset}`);
        ...
        if (missingKeys.length <= 5) {
          ...
          console.log(`   ${colors.yellow}└─ ... и еще ${missingKeys.length - 3} ключей${colors.reset}`);
        }
      }
    }
    ...
    if (moduleHasIssues) {
      ...
      for (const lang of LANGUAGES) {
        ...
        try {
          ...
          console.log(`${colors.green}✅ Обновлен: ${lang}/${module}.json${colors.reset}`);
        } catch (error) {
          console.error(`${colors.red}❌ Ошибка записи: ${filePath}${colors.reset}`);
  ...
  if (totalMissing === 0) {
    console.log(`${colors.green}✨ Все ключи синхронизированы! Проблем не найдено.${colors.reset}`);
  } else {
    console.log(`${colors.yellow}📝 Модулей с проблемами: ${totalModulesWithIssues}/${modules.length}${colors.reset}`);
    console.log(`${colors.yellow}🔍 Всего отсутствующих ключей: ${totalMissing}${colors.reset}`);
    console.log(`${colors.green}✅ Добавлено placeholder'ов: ${totalAdded}${colors.reset}`);
    
    console.log(`\n${colors.blue}По языкам:${colors.reset}`);
    for (const lang of LANGUAGES) {
      const stats = report.summary.byLanguage[lang];
      if (stats.missing > 0) {
        console.log(`  ${lang}: ${colors.red}${stats.missing} отсутствовало${colors.reset}, ${colors.green}${stats.added} добавлено${colors.reset}`);
      }
    }
  }
  ...
  for (const module of modules) {
    for (const lang of LANGUAGES) {
      ...
      try {
        const content = fs.readFileSync(filePath, 'utf8');
        const placeholderMatches = content.match(/\[MISSING_[A-Z]+\]|\[RU\]|\[EN\]|\[SR\]/g);
  ...
  if (placeholderCount > 0) {
    console.log(`${colors.yellow}⚠️  Найдено ${placeholderCount} placeholder'ов в ${placeholderModules.size} файлах${colors.reset}`);
    console.log(`${colors.yellow}   Используйте 'npm run clean-placeholders' для их поиска${colors.reset}`);
  } else {
