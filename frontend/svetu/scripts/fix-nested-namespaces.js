...
const nestedNamespacePattern = /useTranslations\s*\(\s*['"`]([^'"`]+\.[^'"`]+)['"`]\s*\)/g;
...
console.log(`${colors.blue}Поиск файлов с неправильным использованием namespace...${colors.reset}\n`);
...
files.forEach(file => {
  ...
  if (matches) {
    console.log(`${colors.yellow}Найдено в ${path.relative(process.cwd(), file)}:${colors.reset}`);
    ...
    matches.forEach(match => {
      ...
      console.log(`  ${colors.green}+ useTranslations('${module}')${colors.reset}`);
...
fixedFiles.forEach(file => {
  ...
  const varPattern = /const\s+(\w+)\s*=\s*useTranslations\s*\(\s*['"`](\w+)['"`]\s*\)/g;
  let match;
  ...
  translationVars.forEach(({ varName, module }) => {
    const usagePattern = new RegExp(`${varName}\\s*\\(\\s*['"\`]([^'"\`]+\\.[^'"\`]+)['"\`]`, 'g');
    const usages = content.match(usagePattern);
    
    if (usages) {
      console.log(`${colors.yellow}⚠️  В файле ${path.relative(process.cwd(), file)} нужно проверить использование:${colors.reset}`);
      console.log(`   Модуль '${module}', переменная '${varName}'`);
      console.log(`   Найдены вложенные обращения, которые могут требовать ручной проверки`);
    }
  });
});
...
if (totalFixed > 0) {
  ...
  console.log(`${colors.yellow}Если использовались вложенные ключи типа t('products.title'),${colors.reset}`);
  console.log(`${colors.yellow}их нужно изменить на t('products.title') без изменений.${colors.reset}`);
} else {
  console.log(`${colors.green}Файлов с неправильным использованием namespace не найдено!${colors.reset}`);
}