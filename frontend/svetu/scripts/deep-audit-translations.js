    ...
    if (fs.existsSync(localeDir)) {
      ...
      files.forEach(file => {
        ...
        try {
          ...
          console.error(`${colors.red}Ошибка загрузки ${locale}/${file}: ${e.message}${colors.reset}`);
        }
      });
    }
  });
...
function checkKeyConsistency(translations) {
  ...
  const modules = [...new Set(locales.flatMap(l => Object.keys(translations[l])))];
...
function checkComponentUsage() {
  ...
  const componentFiles = glob.sync(path.join(__dirname, '..', 'src', '**', '*.{tsx,ts,jsx,js}'), {
    ignore: ['**/node_modules/**', '**/scripts/**', '**/*.test.*', '**/*.spec.*']
  });
  ...
  componentFiles.forEach(file => {
    ...
    const nestedNamespace = content.match(/useTranslations\s*\(\s*['"`]([^'"`]+\.[^'"`]+)['"`]\s*\)/g);
    if (nestedNamespace) {
    ...
    const useTranslationsMatch = content.match(/const\s+(\w+)\s*=\s*useTranslations\s*\(\s*['"`](\w+)['"`]\s*\)/g);
    if (useTranslationsMatch) {
      useTranslationsMatch.forEach(match => {
        const [, varName, module] = match.match(/const\s+(\w+)\s*=\s*useTranslations\s*\(\s*['"`](\w+)['"`]\s*\)/);
        
        // Ищем использование переводов
        const usageRegex = new RegExp(`${varName}\\s*\\(\\s*['"\`]([^'"\`]+)['"\`]`, 'g');
        let usage;
  ...
  console.log(`Найдено ${problems.emptyTranslations.length} файлов с пустым useTranslations()`);
  console.log(`Найдено ${problems.wrongUsage.length} неправильных использований namespace`);
}
      ...
      keys.forEach(key => {
        if (key.includes('_')) {
          problems.snakeCaseKeys.push({
            ...
            module,
            key,
            suggestion: key.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase())
          });
        }
      });
    });
...
function printReport() {
  ...
  totalProblems = Object.values(problems).reduce((sum, arr) => sum + arr.length, 0);
  
  if (totalProblems === 0) {
    console.log(`${colors.green}✅ Проблем не найдено! Система переводов в порядке.${colors.reset}`);
    return;
  }
  
  console.log(`${colors.red}Всего найдено проблем: ${totalProblems}${colors.reset}\n`);
  
  // Вывод отсутствующих ключей
  if (problems.missingKeys.length > 0) {
    console.log(`${colors.red}❌ Отсутствующие ключи (${problems.missingKeys.length})${colors.reset}`);
  ...
  if (problems.wrongUsage.length > 0) {
    console.log(`${colors.red}❌ Неправильное использование namespace (${problems.wrongUsage.length})${colors.reset}`);
  ...
  if (problems.emptyTranslations.length > 0) {
    console.log(`${colors.red}❌ Пустые useTranslations() (${problems.emptyTranslations.length} файлов)${colors.reset}`);
    problems.emptyTranslations.slice(0, 5).forEach(p => {
      console.log(`  - ${p.file} (${p.count} раз)`);
    });
    if (problems.emptyTranslations.length > 5) {
      console.log(`  ... и еще ${problems.emptyTranslations.length - 5} файлов`);
    }
    console.log();
  }
  ...
  if (problems.snakeCaseKeys.length > 0) {
    console.log(`${colors.yellow}⚠️  Snake_case ключи (${problems.snakeCaseKeys.length})${colors.reset}`);
    problems.snakeCaseKeys.slice(0, 5).forEach(p => {
      console.log(`  - ${p.locale}/${p.module}: "${p.key}" → "${p.suggestion}"`);
    });
...
async function checkBrowserErrors() {
  ...
  console.log('Откройте http://localhost:3001 и проверьте консоль браузера на наличие ошибок переводов');
...
// Основная функция
async function main() {
  console.log(`${colors.blue}Запуск глубокого аудита системы переводов...${colors.reset}`);
