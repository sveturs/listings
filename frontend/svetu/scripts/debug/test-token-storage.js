...
console.log(
  'svetu_refresh_token:',
  sessionStorage.getItem('svetu_refresh_token')?.substring(0, 50) + '...'
);
...
const testRefreshToken =
  'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.test_refresh_token';
...
if (window.tokenManager) {
  console.log(
    'Access token from TokenManager:',
    window.tokenManager.getAccessToken()?.substring(0, 50) + '...'
  );
  console.log(
    'Refresh token from TokenManager:',
    window.tokenManager.getRefreshToken()?.substring(0, 50) + '...'
  );
} else {
