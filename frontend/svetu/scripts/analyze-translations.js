...
async function main() {
  ...
  for (const lang of languages) {
    ...
    if (oldSystem) {
      ...
      console.log(`  Размер: ${formatSize(oldSystem.raw)} (gzip: ${formatSize(oldSystem.gzipped)})`);
    ...
    const sortedModules = Object.entries(modularSystem.modules)
      .sort((a, b) => b[1].raw - a[1].raw);
    
    sortedModules.forEach(([name, data]) => {
      const percentage = ((data.raw / modularSystem.totalRaw) * 100).toFixed(1);
      console.log(`  ${name.padEnd(20)} ${formatSize(data.raw).padEnd(10)} (${percentage}%) - ${data.keyCount} ключей`);
    });
    
    console.log(`  ${'─'.repeat(50)}`);
    console.log(`  ${'ИТОГО:'.padEnd(20)} ${formatSize(modularSystem.totalRaw).padEnd(10)} (gzip: ${formatSize(modularSystem.totalGzipped)})`);
  ...
  if (langStats.modularSystem) {
    scenarios.forEach(scenario => {
      ...
      const savingsPercent = oldSize ? ((savings / oldSize) * 100).toFixed(1) : 0;
      
      console.log(`${scenario.name}:`);
      console.log(`  Модули: ${scenario.modules.join(', ')}`);
      console.log(`  Размер: ${formatSize(pageGzipSize)} (gzip)`);
      if (oldSize) {
        console.log(`  Экономия: ${colors.green}${formatSize(savings)} (${savingsPercent}%)${colors.reset}`);
      }
      console.log('');
    });
  }
  ...
  console.log('1. Перенесите оставшиеся переводы из misc.json в соответствующие модули');
  ...
  console.log('5. Рассмотрите использование Service Worker для offline доступа');
}
