...
function fixAllSyntaxErrors(filePath) {
  ...
  const patterns = [
    ...
    {
      regex:
        /const tCreate_storefront\.[a-zA-Z_\.]+ = useTranslations\('create_storefront'\);/g,
      replacement:
        "const tCreate_storefront = useTranslations('create_storefront');",
    },
  ...
  lines.forEach((line, index) => {
    const match = line.match(
      /^\s*const\s+(t|tCommon|tAdmin|tStorefronts|tCreate_listing|tCreate_storefront|tProfile|tRoles|tPermissions)\s*=\s*useTranslations\('([^']+)'\);?\s*$/
    );
    if (match) {
