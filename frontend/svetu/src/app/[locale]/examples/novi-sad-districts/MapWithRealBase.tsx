...
const MapWithRealBase: React.FC<MapWithRealBaseProps> = ({
  ...
  useEffect(() => {
    ...
    districts.forEach((district) => {
      try {
        // Фильтруем и проверяем валидность координат
        const validOriginalCoords = district.coordinates.filter(
          (coord) =>
            coord &&
            coord.length >= 2 &&
        ...
        if (validOriginalCoords.length === 0) {
          issues.push(`${district.name}: нет валидных координат`);
          return;
        }

        // Convert coordinates to Leaflet format [lat, lng]
        const leafletCoords = validOriginalCoords.map((coord) => [
          coord[1],
          coord[0],
        ]);

        // Check if coordinates are valid for Novi Sad area
        const validCoords = leafletCoords.every(
          (coord) =>
            coord[0] >= 44 &&
            coord[0] <= 46 && // Valid latitude for Novi Sad area
            coord[1] >= 19 &&
            coord[1] <= 21 // Valid longitude for Novi Sad area
        );
  ...
  useEffect(() => {
    if (leafletMapRef.current && selectedDistrict && mapReady) {
      ...
      // Фильтруем и проверяем валидность координат
      const validCoords = selectedDistrict.coordinates.filter(
        (coord) =>
          coord &&
          coord.length >= 2 &&
          typeof coord[0] === 'number' &&
      ...
      if (validCoords.length > 0) {
        const leafletCoords = validCoords.map((coord) => [coord[1], coord[0]]);
        ...
        try {
          ...
          if (bounds.isValid()) {
            ...
            console.warn('Invalid bounds for district:', selectedDistrict.name);
          }
        } catch (error) {
          console.error(
            'Error creating bounds for district:',
        ...
        console.warn(
          'No valid coordinates found for district:',
          selectedDistrict.name
        );
      }
    }
