  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setError(error.message || t('storefronts.products.errors.loadFailed'));
      } finally {
        setIsLoading(false);
      ...
      <div className="container mx-auto px-4 py-8 flex justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-base-content/70">
            {t('storefronts.products.loadingProduct')}
          </p>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-error/10 border border-error rounded-2xl p-6 text-center">
          <h3 className="text-lg font-semibold text-error mb-2">
            {t('storefronts.products.errors.loadFailed')}
          </h3>
          ...
          >
            {t('storefronts.products.backToProducts')}
          </Link>
        </div>
      </div>
    );
  ...
  if (!state.originalProduct) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-error/10 border border-error rounded-2xl p-6 text-center">
          <h3 className="text-lg font-semibold text-error mb-2">
            {t('storefronts.products.errors.productNotFound')}
          </h3>
          <Link
            href={`/storefronts/${slug}/products`}
            className="btn btn-primary"
      ...
      <div className="bg-base-100 shadow-sm border-b">
        <div className="container mx-auto px-4 py-6">
          ...
          <h1 className="text-3xl font-bold text-base-content">
            {t('storefronts.products.editProduct')}
          </h1>
          <p className="text-base-content/70 mt-2">
            {t('storefronts.products.editProductDescription')}
          </p>
        </div>
      </div>
