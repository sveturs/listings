...
export default function StorefrontProductRedirect() {
    ...
    fetch(`${apiUrl}/api/v1/storefronts/products/${productId}`, {
      ...
      .then((result) => {
        ...
        if (data && data.storefront) {
          ...
          router.replace(
            `/${locale}/storefronts/${data.storefront.slug}/products/${productId}`
          );
        } else {
          // Fallback - используем дефолтный slug
          router.replace(`/${locale}/storefronts/store/products/${productId}`);
        }
      })
      .catch((err) => {
        console.error('Error loading product:', err);
        // В случае ошибки редиректим на главную страницу витрин
        router.replace(`/${locale}/storefronts`);
      });
