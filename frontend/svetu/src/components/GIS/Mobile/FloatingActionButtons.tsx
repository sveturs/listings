...
const FloatingActionButtons: React.FC<FloatingActionButtonsProps> = ({
  ...
  return (
    <div className="fixed top-4 right-4 flex flex-col items-end gap-3 z-30 md:hidden">
      ...
      >
        ...
        <button
          ...
          title={t('geolocation.title', 'Моё местоположение')}
        >
          <svg
            className="w-5 h-5"
            fill="none"
      ...
      {markersCount > 0 && (
        <button
          ...
          className="fixed bottom-4 left-4 right-4 bg-blue-600 hover:bg-blue-700 text-white py-4 px-6 rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 flex items-center justify-center gap-3 font-medium z-20"
        >
          <svg
            className="w-5 h-5"
            fill="none"
          ...
          <span>
            {t('results.showCount', 'Показать результаты')} ({markersCount})
          </span>
        </button>
      )}

      {/* Компактный поиск сверху */}
      <div className="fixed top-4 left-4 right-20 z-20">
        <div className="bg-white/90 backdrop-blur-sm rounded-full shadow-lg px-4 py-3 flex items-center gap-3">
          <svg
          ...
          <button
            ...
            className="flex-1 text-left text-gray-500 text-sm"
          >
            {t('search.placeholder', 'Поиск по адресу...')}
          </button>
          {isLoading && (
