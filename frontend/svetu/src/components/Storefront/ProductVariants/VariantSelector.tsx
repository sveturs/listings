...
export default function VariantSelector({ 
  ...
  const loadVariants = async () => {
    try {
      const response = await fetch(`/api/v1/storefront/products/${productId}/variants`);
      if (response.ok) {
        ...
        // Set default variant
        const defaultVariant = variantData.find((v: ProductVariant) => v.is_default) || variantData[0];
        if (defaultVariant) {
  ...
  const findMatchingVariant = (attributes: Record<string, string>): ProductVariant | null => {
  ...
  const getMainImage = (): string | null => {
    if (selectedVariant?.images?.length > 0) {
      const mainImage = selectedVariant.images.find(img => img.is_main) || selectedVariant.images[0];
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-medium ${
            getStockStatus() === 'in_stock' ? 'text-green-600' :
            getStockStatus() === 'low_stock' ? 'text-yellow-600' :
            'text-red-600'
          }`}>
      ...
      {selectedVariant?.images && selectedVariant.images.length > 1 && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-gray-700">{t('more_images')}</h4>
