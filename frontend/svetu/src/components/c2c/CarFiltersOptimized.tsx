...
interface CarFiltersOptimizedProps {
  onFiltersChange: (filters: Record<string, any>) => void;
  className?: string;
}

export const CarFiltersOptimized: React.FC<CarFiltersOptimizedProps> = ({
  onFiltersChange,
  className = '',
}) => {
  ...
  const { data: models = [], isLoading: loadingModels } = useCarModels(
    selectedMake,
    !!selectedMake
  );

  // Сброс модели при изменении марки
  useEffect(() => {
    if (!selectedMake) {
      setSelectedModel('');
    }
    ...
    <div className={`card bg-base-100 shadow-xl ${className}`}>
      <div className="card-body">
        ...
        <div className="space-y-4">
          {/* Марка и модель с индикатором загрузки */}
...
export default CarFiltersOptimized;
