...
import { C2CItem, MarketplaceImage } from '@/types/c2c';
...
export const EnhancedListingCard: React.FC<EnhancedListingCardProps> = memo(
  function EnhancedListingCard({
    ...
    // Мемоизируем вычисление condition из атрибутов
    const itemCondition = useMemo(() => {
      ...
      return conditionAttr
        ? String(conditionAttr.value || conditionAttr.text_value || '')
        : undefined;
    }, [item.condition, item.attributes]);
    ...
    // Мемоизируем словарь бейджей
    const conditionBadge = useMemo<
      Record<string, { text: string; class: string }>
    >(
      () => ({
    ...
    // Мемоизируем функцию форматирования цены
    ...
    const getImageUrl = useCallback((image?: MarketplaceImage) => {
    ...
    const handleChatClick = useCallback(
      ...
      [isAuthenticated, locale, router, item.user_id, item.id, user?.id]
    );

    const isOnlinePurchaseAvailable = useCallback(() => {
    ...
    const handleBuyClick = useCallback(
      (e: React.MouseEvent) => {
        ...
        if (isOnlinePurchaseAvailable()) {
          ...
          router.push(`/${locale}/c2c/${item.id}/buy`);
        }
      },
      [
        isAuthenticated,
    ...
    const handlePriceHistoryClick = useCallback((e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    const getItemUrl = useCallback(() => {
      ...
      if (item.product_type === 'storefront' && item.storefront_slug) {
        return `/${locale}/b2c/${item.storefront_slug}/products/${item.id}`;
      }
      // Все объявления маркетплейса открываются через marketplace URL,
      // даже если они связаны с витриной
      return `/${locale}/c2c/${item.id}`;
    }, [item.product_type, item.storefront_slug, item.id, locale]);

    // Мемоизируем эко-скор для б/у товаров
