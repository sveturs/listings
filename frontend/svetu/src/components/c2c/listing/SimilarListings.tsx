...
export default function SimilarListings({ listingId }: SimilarListingsProps) {
      ...
      try {
        ...
        if (items.length > 0) {
          ...
          const filteredListings = items.filter(
            (item: C2CItem) => item.id !== listingId
          );
  ...
  const handleChatClick = (e: React.MouseEvent, listing: C2CItem) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleBuyClick = (e: React.MouseEvent, listing: C2CItem) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    router.push(`/${locale}/c2c/${listing.id}/buy`);
  };

  const isOnlinePurchaseAvailable = (listing: C2CItem) => {
