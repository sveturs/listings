...
export class TokenMigration {
  ...
  static hasOldTokens(): boolean {
    ...
    for (const cookie of cookies) {
      ...
      if (this.OLD_TOKEN_KEYS.includes(name) && value) {
        if (this.isHS256Token(value)) {
          console.log(
            `[TokenMigration] Found old HS256 token in cookie: ${name}`
          );
          return true;
        }
      }
    ...
    for (const key of this.OLD_TOKEN_KEYS) {
      ...
      if (token && this.isHS256Token(token)) {
        console.log(
          `[TokenMigration] Found old HS256 token in localStorage: ${key}`
        );
        return true;
      }
    }
    ...
    for (const key of this.OLD_TOKEN_KEYS) {
      ...
      if (token && this.isHS256Token(token)) {
        console.log(
          `[TokenMigration] Found old HS256 token in sessionStorage: ${key}`
        );
        return true;
      }
    }
  ...
  static clearAllAuthData(): void {
    ...
    document.cookie.split(';').forEach((c) => {
      ...
      if (
        this.OLD_TOKEN_KEYS.includes(name) ||
        name === 'user_id' ||
        name === 'user_email' ||
        name === 'session_id'
      ) {
  ...
  static runMigration(): boolean {
    ...
    if (this.hasOldTokens()) {
      console.log(
        '[TokenMigration] Old HS256 tokens detected, performing migration...'
      );
      ...
      console.log(
        '[TokenMigration] Migration completed. User needs to re-authenticate.'
      );
