...
export function forceTokenCleanup(): boolean {
  ...
  document.cookie.split(';').forEach((cookie) => {
    ...
    if (cookiesToClear.includes(name) || name.includes('token') || name.includes('auth')) {
  ...
  localStorageKeys.forEach(key => {
    if (key.includes('token') || key.includes('auth') || key.includes('user') || 
        key === 'access_token' || key === 'refresh_token' || key === 'jwt_token') {
  ...
  sessionStorageKeys.forEach(key => {
    ...
    if (key === 'token_migration_v2_done' || key === 'force_token_cleanup_done') return;
    ...
    if (key.includes('token') || key.includes('auth') || key.includes('user') || key.includes('jwt') || key.includes('session')) {
      ...
      if (value && value.includes('.')) {
        try {
          ...
          if (header.alg === 'HS256') {
            ...
            console.log(`[ForceTokenCleanup] Cleared HS256 token from sessionStorage: ${key}`);
            cleanedSomething = true;
            ...
            sessionStorage.removeItem(key);
            console.log(`[ForceTokenCleanup] Cleared non-svetu RS256 token: ${key}`);
            cleanedSomething = true;
          }
        } catch {
          // Not a valid JWT, remove it
          sessionStorage.removeItem(key);
          console.log(`[ForceTokenCleanup] Cleared invalid token from sessionStorage: ${key}`);
          cleanedSomething = true;
        }
      } else {
  ...
  if (cleanedSomething) {
    ...
    sessionStorage.setItem('force_token_cleanup_done', new Date().toISOString());
  } else {
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  console.log('[ForceTokenCleanup] Module loaded, auto-cleanup DISABLED to preserve OAuth tokens');
  console.log('[ForceTokenCleanup] Call forceTokenCleanup() manually when needed');
}