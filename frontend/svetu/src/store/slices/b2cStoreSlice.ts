...
export interface StorefrontState {
  // Основные данные
  b2cStores: B2CStore[];
  currentStorefront: B2CStore | null;
  myStorefronts: B2CStore[];
...
const storefrontSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentStorefront: (state, action: PayloadAction<B2CStore | null>) => {
    ...
    updateStorefrontInLists: (state, action: PayloadAction<B2CStore>) => {
      const updatedStorefront = action.payload;

      // Обновляем в общем списке
      const storefrontIndex = state.b2cStores.findIndex(
        (s) => s.id === updatedStorefront.id
      );
      if (storefrontIndex !== -1) {
        state.b2cStores[storefrontIndex] = updatedStorefront;
      }
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchStorefronts.fulfilled, (state, action) => {
        ...
        state.hasMore = state.b2cStores.length < total;
      })
    ...
    builder
      ...
      .addCase(createStorefront.fulfilled, (state, action) => {
        ...
        if (state.b2cStores.length > 0) {
          state.b2cStores.unshift(action.payload);
          state.totalCount += 1;
        }
      })
    ...
    builder
      ...
      .addCase(deleteStorefront.fulfilled, (state, action) => {
        ...
        state.b2cStores = state.b2cStores.filter((s) => s.id !== deletedId);
...
export const selectIsUpdating = (state: RootState) =>
  state.b2cStores.isUpdating;
export const selectIsDeleting = (state: RootState) =>
  state.b2cStores.isDeleting;
export const selectError = (state: RootState) => state.b2cStores.error;
export const selectFilters = (state: RootState) => state.b2cStores.filters;
export const selectPagination = (state: RootState) =>
  state.b2cStores.pagination;
