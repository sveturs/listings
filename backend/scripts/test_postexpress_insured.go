...
// Тест 2: Отправление с ОБЪЯВЛЕННОЙ ЦЕННОСТЬЮ и страховкой
...
func main() {
	fmt.Println("🚀 Post Express Test - Insured Value (Объявленная ценность)")
	fmt.Println("==========================================================\n")
	...
	fmt.Println("📦 ТЕСТ: Отправление с объявленной ценностью 10000 RSD + страховка")
	fmt.Println("-------------------------------------------------------------------")
				...
				if porudzbina, ok := porudzbine[0].(map[string]interface{}); ok {
					if posiljke, ok := porudzbina["Posiljke"].([]interface{}); ok && len(posiljke) > 0 {
						if posiljka, ok := posiljke[0].(map[string]interface{}); ok {
							...
							fmt.Printf("💎 Объявленная ценность: 10000 RSD\n")
							fmt.Printf("🛡️ Услуга: VD (Vrednosna dostava)\n")
