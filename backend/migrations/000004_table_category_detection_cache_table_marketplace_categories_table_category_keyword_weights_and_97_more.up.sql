...
CREATE TABLE public.category_proposals (
    ...
    description text,
    reasoning text,
    expected_products integer DEFAULT 0,
    external_category_source character varying(255),
    similar_categories integer[] DEFAULT ARRAY[]::integer[],
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT category_proposals_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'approved'::character varying, 'rejected'::character varying])::text[])))
);
...
CREATE TABLE public.import_jobs (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT import_jobs_file_type_check CHECK (((file_type)::text = ANY ((ARRAY['xml'::character varying, 'csv'::character varying, 'zip'::character varying])::text[]))),
    CONSTRAINT import_jobs_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'processing'::character varying, 'completed'::character varying, 'failed'::character varying, 'cancelled'::character varying])::text[])))
);
...
CREATE TABLE public.storefront_category_mappings (
    id integer NOT NULL,
    storefront_id integer NOT NULL,
