...
CREATE TABLE public.unified_attributes (
    ...
    CONSTRAINT unified_attributes_attribute_type_check CHECK (((attribute_type)::text = ANY ((ARRAY['text'::character varying, 'textarea'::character varying, 'number'::character varying, 'boolean'::character varying, 'select'::character varying, 'multiselect'::character varying, 'date'::character varying, 'color'::character varying, 'size'::character varying])::text[]))),
    CONSTRAINT unified_attributes_purpose_check CHECK (((purpose)::text = ANY ((ARRAY['regular'::character varying, 'variant'::character varying, 'both'::character varying])::text[])))
)
