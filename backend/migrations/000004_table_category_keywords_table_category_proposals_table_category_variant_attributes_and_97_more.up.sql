...
CREATE TABLE public.import_jobs (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT storefront_import_jobs_file_type_check CHECK (((file_type)::text = ANY ((ARRAY['xml'::character varying, 'csv'::character varying, 'zip'::character varying])::text[]))),
    CONSTRAINT storefront_import_jobs_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'processing'::character varying, 'completed'::character varying, 'failed'::character varying])::text[])))
);
