...
func (s *Storage) ExpandSearchQuery(ctx context.Context, query string, language string) (string, error) {
	var expandedQuery string

	sqlQuery := `SELECT expand_search_query($1, $2)`

	err := s.pool.QueryRow(ctx, sqlQuery, query, language).Scan(&expandedQuery)
	if err != nil {
...
func (s *Storage) SearchCategoriesFuzzy(ctx context.Context, searchTerm string, language string, similarityThreshold float64) ([]interface{}, error) {
	if language == "" {
		language = "ru"
	}
	...
	rows, err := s.pool.Query(ctx, sqlQuery, searchTerm, language, similarityThreshold)
	if err != nil {
