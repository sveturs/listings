// backend/internal/proj/marketplace/service/translation_factory.go
package service
...
func NewTranslationServiceFactory(googleAPIKey, openAIAPIKey string, storage storage.Storage) (*TranslationServiceFactory, error) {
    ...
    if googleAPIKey != "" {
        ...
        if err != nil {
            log.Printf("Не удалось создать сервис Google Translate: %v. Будет использован только OpenAI", err)
            defaultProvider = OpenAI
        }
    } else {
        log.Printf("Google Translate API ключ не предоставлен. Будет использован только OpenAI")
        defaultProvider = OpenAI
    }
    ...
    if openAIAPIKey != "" {
        ...
        if err != nil {
            log.Printf("Не удалось создать сервис OpenAI: %v", err)
        ...
        log.Printf("OpenAI API ключ не предоставлен. Будет использован только Google Translate")
    ...
    log.Printf("Создана фабрика сервисов перевода. По умолчанию используется: %s", defaultProvider)
...
func (f *TranslationServiceFactory) SetDefaultProvider(provider TranslationProvider) error {
    ...
    log.Printf("Установлен провайдер перевода по умолчанию: %s", provider)
    return nil
}
