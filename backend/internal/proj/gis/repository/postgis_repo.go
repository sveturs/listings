...
func (r *PostGISRepository) GetClusters(ctx context.Context, params types.ClusterParams) ([]types.Cluster, []types.GeoListing, error) {
	...
	// Получаем некластеризованные объявления
	// (одиночные объявления или при высоком уровне зума)
	var listings []types.GeoListing
	if params.ZoomLevel >= 15 { // На высоких уровнях зума показываем отдельные объявления
		...
		listings, _, err = r.SearchListings(ctx, searchParams)
		if err != nil {
