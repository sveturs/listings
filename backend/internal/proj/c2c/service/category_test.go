...
func (suite *CategoryIntegrationTestSuite) createSchema() error {
	...
	CREATE TABLE IF NOT EXISTS category_attribute_mapping (
		id SERIAL PRIMARY KEY,
		category_id INTEGER NOT NULL REFERENCES c2c_categories(id),
	...
	CREATE TABLE IF NOT EXISTS c2c_listings (
		...
		category_id INTEGER REFERENCES c2c_categories(id),
	...
	WITH RECURSIVE category_tree AS (
		SELECT id, parent_id FROM c2c_categories
	),
	listing_counts AS (
...
func (suite *CategoryIntegrationTestSuite) TestCreateCategoryWithTranslations() {
	...
	var count int
	err = suite.db.QueryRow("SELECT COUNT(*) FROM c2c_categories WHERE id = $1", id).Scan(&count)
	assert.NoError(suite.T(), err)
...
func (suite *CategoryIntegrationTestSuite) TestUpdateCategoryWithCacheInvalidation() {
	...
	err = suite.db.QueryRow(
		"SELECT name, seo_title FROM c2c_categories WHERE id = $1",
		id,
	).Scan(&name, &seoTitle)
	assert.NoError(suite.T(), err)
...
func (suite *CategoryIntegrationTestSuite) TestDeleteCategoryWithSubcategories() {
	...
	var count int
	err = suite.db.QueryRow("SELECT COUNT(*) FROM c2c_categories WHERE id IN ($1, $2)", parentID, childID).Scan(&count)
	assert.NoError(suite.T(), err)
...
func (suite *CategoryIntegrationTestSuite) TestDeleteCategoryWithListings() {
	...
	_, err = suite.db.Exec(
		"INSERT INTO c2c_listings (title, description, price, category_id, user_id) VALUES ($1, $2, $3, $4, $5)",
	...
	// Удаляем объявление
	_, err = suite.db.Exec("DELETE FROM c2c_listings WHERE category_id = $1", categoryID)
	require.NoError(suite.T(), err)
...
func (suite *CategoryIntegrationTestSuite) TestMoveCategory() {
	...
	var parentID int
	err = suite.db.QueryRow("SELECT parent_id FROM c2c_categories WHERE id = $1", childID).Scan(&parentID)
	assert.NoError(suite.T(), err)
...
func (ts *testStorage) GetCategories(ctx context.Context) ([]models.MarketplaceCategory, error) {
	rows, err := ts.Query(ctx, `
		SELECT id, name, slug, parent_id, icon, has_custom_ui, custom_ui_component, 
		       description, is_active, seo_title, seo_description, seo_keywords, sort_order
		FROM c2c_categories WHERE is_active = true
...
func (ts *testStorage) GetCarListingsCount(ctx context.Context) (int, error) {
	var count int
	err := ts.QueryRow(ctx, `SELECT COUNT(*) FROM c2c_listings WHERE category_id = 1301`).Scan(&count)
	return count, err
}
