...
func (s *MarketplaceService) DeleteCategory(ctx context.Context, id int) error {
	...
	_, err = s.storage.Exec(ctx, "DELETE FROM c2c_categories WHERE id = $1", id)
	if err != nil {
...
func (s *MarketplaceService) MoveCategory(ctx context.Context, id int, newParentID int) error {
	...
	_, err = s.storage.Exec(ctx, "UPDATE c2c_categories SET parent_id = $1 WHERE id = $2", newParentID, id)
	if err != nil {
