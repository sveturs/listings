...
# Скрипт для загрузки образа mail-webui (roundcube) в Harbor с увеличенным таймаутом и механизмом повторных попыток

echo "Начинаем загрузку образа mail-webui в Harbor..."
...
SOURCE_IMAGE="roundcube/roundcubemail:latest"
TARGET_IMAGE="$HARBOR_URL/svetu/mail/webui:latest"
...
if curl -u "$HARBOR_USER:$HARBOR_PASSWORD" -X GET -s "https://$HARBOR_URL/api/v2.0/projects/svetu/repositories/mail%2Fwebui/artifacts/latest" | grep -q "digest"; then
  echo "✅ Образ mail-webui успешно загружен и доступен в Harbor!"
else
  echo "❌ Не удалось проверить наличие образа в Harbor. Возможно, загрузка не завершена или произошла ошибка."
  exit 1
fi

echo "Загрузка mail-webui завершена!"