...
# Обновляем файл docker-compose.prod.yml.harbor для использования образов из Harbor
...
# Запускаем дополнительные сервисы из Harbor
echo "Запускаем дополнительные сервисы из Harbor..."
...
echo "Запускаем оставшиеся сервисы..."
...
echo "Используются образы Harbor для: db, backend, opensearch, minio, createbuckets, migrate, nginx, certbot"
echo "Используются стандартные образы для: mailserver, mail-webui"