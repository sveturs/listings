...
0 0 * * * root certbot renew --quiet --post-hook "cp /etc/letsencrypt/live/$HARBOR_DOMAIN/fullchain.pem $HARBOR_DIR/certs/server.crt && cp /etc/letsencrypt/live/$HARBOR_DOMAIN/privkey.pem $HARBOR_DIR/certs/server.key && cd $HARBOR_DIR && docker compose restart nginx"
EOF
