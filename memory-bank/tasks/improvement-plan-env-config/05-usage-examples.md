...
### 1. Примеры кода работают
- [x] Все примеры из этого файла скопированы и протестированы
- [x] Server Component пример работает без ошибок
- [x] Client Component пример работает без ошибок
- [x] Feature flags корректно включают/выключают функциональность
- [x] WebSocket подключается при наличии NEXT_PUBLIC_WEBSOCKET_URL

### 2. Конфигурация для окружений
- [x] Приложение работает с дефолтными переменными (без .env.local)
- [x] Приложение работает с кастомными переменными из .env.local
- [x] При изменении переменных и перезапуске значения обновляются:
  ```bash
  NEXT_PUBLIC_ENABLE_PAYMENTS=true yarn dev
  NEXT_PUBLIC_ENABLE_PAYMENTS=false yarn dev
  ```

### 3. Health check endpoint
- [x] Endpoint `/api/health` создан и доступен
- [x] Возвращает статус конфигурации и проверок
- [x] При невалидной конфигурации возвращает 503 статус
- [x] Проверяет доступность API и Storage

### 4. Компоненты с feature flags
- [x] PaymentButton отображается только при `NEXT_PUBLIC_ENABLE_PAYMENTS=true`
- [x] Debug информация показывается только в development режиме
- [x] WebSocket подключается только при наличии URL

### 5. Интеграционное тестирование
- [x] Полная проверка проходит успешно:
  ```bash
  yarn format && yarn lint && yarn build
  ```
- [x] Приложение собирается без ошибок TypeScript
- [x] Нет предупреждений о неиспользуемых переменных
- [x] Bundle size не увеличился более чем на 10kb

### 6. Миграция существующего кода
- [x] Checklist миграции актуален и полон
- [x] Существующие компоненты продолжают работать
- [x] Новые компоненты могут использовать runtime конфигурацию
- [x] Нет breaking changes в API

### 7. Документация и примеры
- [x] Примеры покрывают основные use cases:
  ...
  - Middleware
  - Testing
- [x] Код хорошо комментирован
- [x] Есть примеры для разных окружений (dev, Docker, Kubernetes)

### 8. Мониторинг и отладка
- [x] В development режиме легко увидеть текущую конфигурацию
- [x] Ошибки конфигурации логируются понятно
- [x] Health check предоставляет полезную информацию для диагностики

## Статус выполнения

✅ **Задача завершена успешно!**

Все примеры использования новой системы конфигурации созданы и протестированы. Приложение полностью поддерживает runtime конфигурацию и готово к развертыванию с разными настройками окружения без необходимости пересборки.