...
- [ ] Все примеры из этого файла скопированы и протестированы
- [ ] Server Component пример работает без ошибок
- [ ] Client Component пример работает без ошибок
- [ ] Feature flags корректно включают/выключают функциональность
- [ ] WebSocket подключается при наличии NEXT_PUBLIC_WEBSOCKET_URL

### 2. Конфигурация для окружений
- [ ] Приложение работает с дефолтными переменными (без .env.local)
- [ ] Приложение работает с кастомными переменными из .env.local
- [ ] При изменении переменных и перезапуске значения обновляются:
  ```bash
...
- [ ] Endpoint `/api/health` создан и доступен
- [ ] Возвращает статус конфигурации и проверок
- [ ] При невалидной конфигурации возвращает 503 статус
- [ ] Проверяет доступность API и Storage

### 4. Компоненты с feature flags
- [ ] PaymentButton отображается только при `NEXT_PUBLIC_ENABLE_PAYMENTS=true`
- [ ] Debug информация показывается только в development режиме
- [ ] WebSocket подключается только при наличии URL

### 5. Интеграционное тестирование
- [ ] Полная проверка проходит успешно:
  ```bash
...
- [ ] Bundle size не увеличился более чем на 10kb

### 6. Миграция существующего кода
- [ ] Checklist миграции актуален и полон
- [ ] Существующие компоненты продолжают работать
- [ ] Новые компоненты могут использовать runtime конфигурацию
- [ ] Нет breaking changes в API

### 7. Документация и примеры
- [ ] Примеры покрывают основные use cases:
...
- [ ] Есть примеры для разных окружений (dev, Docker, Kubernetes)

### 8. Мониторинг и отладка
- [ ] В development режиме легко увидеть текущую конфигурацию
- [ ] Ошибки конфигурации логируются понятно
- [ ] Health check предоставляет полезную информацию для диагностики