...
- [ ] `next-runtime-env` добавлен в package.json
- [ ] yarn.lock обновлен после установки
- [ ] Библиотека установлена локально (проверить через `yarn list next-runtime-env`)

### 2. Интеграция в layout
- [ ] `PublicEnvScript` импортирован в `/src/app/[locale]/layout.tsx`
- [ ] Скрипт добавлен в `<head>` секцию до других скриптов
- [ ] Приложение запускается без ошибок (`yarn dev -p 3001`)

### 3. Создание утилиты env.ts
- [ ] Файл `/src/utils/env.ts` создан
- [ ] Функция `getEnv()` корректно определяет контекст (server/client)
- [ ] Объект `publicEnv` предоставляет типизированный доступ к переменным
- [ ] TypeScript не выдает ошибок при импорте и использовании

### 4. Проверка работоспособности
- [ ] Тестовая страница `/test-env` создана и доступна по адресу http://localhost:3001/test-env
- [ ] Страница отображает значения переменных окружения
- [ ] При изменении переменных и перезапуске сервера значения обновляются:
  ```bash
  NEXT_PUBLIC_API_URL=https://test.com yarn dev -p 3001
  ```
- [ ] В браузере в консоли доступен объект `window.__ENV` с переменными
- [ ] Нет ошибок гидратации React в консоли браузера

### 5. Дополнительные проверки
- [ ] Переменные без префикса `NEXT_PUBLIC_` не попадают в браузер
- [ ] Runtime переменные работают как в dev, так и в production режиме
- [ ] Существующий функционал приложения не сломан