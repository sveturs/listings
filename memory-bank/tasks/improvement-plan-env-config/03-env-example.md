...
### 1. Файлы созданы
- [ ] `/frontend/svetu/.env.example` создан
- [ ] `/frontend/svetu/.env.production.example` создан
- [ ] `/frontend/svetu/docs/ENVIRONMENT.md` создан
- [ ] Все файлы содержат актуальную информацию

### 2. Содержимое .env.example
- [ ] Все используемые в приложении переменные документированы
- [ ] Каждая переменная имеет комментарий с описанием
- [ ] Примеры значений корректны и работоспособны
- [ ] Переменные сгруппированы по категориям (API, Storage, Features и т.д.)

### 3. Скрипт проверки переменных
- [ ] Файл `/frontend/svetu/scripts/check-env.js` создан
- [ ] Скрипт исполняемый (`chmod +x scripts/check-env.js`)
- [ ] Скрипт проверяет обязательные переменные (NEXT_PUBLIC_API_URL, NEXT_PUBLIC_MINIO_URL)
- [ ] Скрипт возвращает exit code 1 при отсутствии обязательных переменных
- [ ] Скрипт выводит понятные сообщения об ошибках и предупреждениях

### 4. Интеграция в package.json
- [ ] Команда `env:check` добавлена и работает
- [ ] Команда `env:create` создает .env.local из .env.example
- [ ] Hook `predev` запускает проверку перед `yarn dev`
- [ ] Hook `prebuild` запускает проверку перед `yarn build`
- [ ] При отсутствии обязательных переменных dev/build не запускается

### 5. Git конфигурация
- [ ] `.gitignore` обновлен
- [ ] `.env.local` не коммитится (проверить `git status`)
- [ ] `.env.example` файлы коммитятся
- [ ] Нет случайно закоммиченных env файлов с реальными значениями

### 6. Использование и документация
- [ ] Новый разработчик может настроить окружение следуя инструкциям:
  ```bash
  yarn env:create
  ...
  ```
- [ ] Документация ENVIRONMENT.md содержит:
...
### 7. Безопасность
- [ ] Production значения не указаны в примерах
- [ ] Есть предупреждения о недопустимости коммита реальных значений
- [ ] Sensitive переменные помечены соответствующим образом