# 🗄️ Database Migration Workflows

Простая система миграций базы данных для продакшн сервера.

## 📦 1. Build Migration Tool

**Файл:** `.github/workflows/migrator-build.yml`

**Что делает:** Собирает Docker образ с мигратором и всеми файлами миграций.

**Запуск:** Ручной через GitHub Actions

**Результат:** 
- `harbor.svetu.rs/svetu/migrator:latest`
- `harbor.svetu.rs/svetu/migrator:TIMESTAMP`

## 🚀 2. Production Database Migration

**Файл:** `.github/workflows/migrate-production.yml`

**Что делает:** Запускает миграции на продакшн сервере.

**Параметры:**
- `migration_direction` - up/down
- `target_version` - конкретная версия миграции (опционально, например: 000050)

**Процесс:**
1. Подключается к серверу по SSH
2. Скачивает последний образ мигратора
3. Запускает контейнер с нужными параметрами
4. Передаёт переменные окружения для подключения к базе

## 🔧 Использование

### Сборка образа мигратора

1. GitHub Actions → Build Migration Tool
2. Run workflow
3. Образ будет собран и загружен в Harbor

### Применение миграций (UP)

#### Все новые миграции
1. GitHub Actions → Production Database Migration
2. Параметры:
   - `migration_direction`: up
   - `target_version`: (оставить пустым)
3. Run workflow

#### До конкретной версии
1. GitHub Actions → Production Database Migration
2. Параметры:
   - `migration_direction`: up
   - `target_version`: 000050 (номер версии)
3. Run workflow

### Откат миграций (DOWN)

#### Откат одной миграции
1. Посмотрите текущую версию в базе
2. GitHub Actions → Production Database Migration
3. Параметры:
   - `migration_direction`: down
   - `target_version`: предыдущая версия
4. Run workflow

#### Откат всех миграций
⚠️ **ВНИМАНИЕ: Откатывает ВСЕ миграции!**

1. GitHub Actions → Production Database Migration  
2. Параметры:
   - `migration_direction`: down
   - `target_version`: (оставить пустым)
3. Run workflow

## 📋 Технические детали

### Переменные окружения на сервере

В файле `/opt/svetu/.env`:
```bash
POSTGRES_USER=...
POSTGRES_PASSWORD=...
POSTGRES_DB=...
```

### Docker команда для миграций

```bash
docker run --rm --network svetu_default \
  -e DATABASE_URL="postgres://..." \
  -e MIGRATIONS_PATH="/migrations" \
  -e MIGRATION_DIRECTION="up" \
  -e MIGRATION_TARGET="000050" \
  harbor.svetu.rs/svetu/migrator:latest
```

### Как работает мигратор

- **Переменные окружения:**
  - `DATABASE_URL` - строка подключения к базе данных
  - `MIGRATIONS_PATH` - путь к директории с миграциями (по умолчанию: "migrations")
  - `MIGRATION_DIRECTION` - направление миграции: up/down (по умолчанию: "up")
  - `MIGRATION_TARGET` - целевая версия миграции (опционально)

- **Возможности:**
  - `up` - применить все новые миграции
  - `up` с `MIGRATION_TARGET` - мигрировать до конкретной версии
  - `down` - откатить ВСЕ миграции
  - `down` с `MIGRATION_TARGET` - откатить до конкретной версии

- **Обратная совместимость:**
  - Поддерживает аргументы командной строки: `migrator up` или `migrator down`
  - Если переменные окружения не заданы, используются значения по умолчанию

## 🔗 Связанные файлы

- `.github/workflows/migrator-build.yml` - сборка образа
- `.github/workflows/migrate-production.yml` - запуск миграций
- `backend/Dockerfile.migrator` - Dockerfile для мигратора
- `backend/migrations/` - файлы миграций

