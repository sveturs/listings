...
func TestAddToFavorites(t *testing.T) {
	...
	t.Run("AddListingToFavorites_Success", func(t *testing.T) {
		...
		// Verify in database
		exists := RowExists(t, server, "listing_favorites", "user_id = $1 AND listing_id = $2", 200, 101)
	...
	t.Run("AddDuplicate_Idempotent", func(t *testing.T) {
		...
		count := CountRows(t, server, "listing_favorites", "user_id = $1 AND listing_id = $2", 201, 102)
...
func TestRemoveFromFavorites(t *testing.T) {
	...
	t.Run("RemoveFavorite_Success", func(t *testing.T) {
		...
		exists := RowExists(t, server, "listing_favorites", "user_id = $1 AND listing_id = $2", 203, 103)
