...
DB_BACKUP_PATH="/tmp/hostel-backup/db_backup.sql"
docker-compose -f docker-compose.prod.yml exec -T db pg_dump -U postgres hostel_db > $DB_BACKUP_PATH
...
if [ -f "$DB_BACKUP_PATH" ]; then
    ...
    cat $DB_BACKUP_PATH | docker-compose -f docker-compose.prod.yml exec -T db psql -U postgres hostel_db
    echo "Database restored successfully!"
else
    echo "Database backup not found, skipping restore."
fi
